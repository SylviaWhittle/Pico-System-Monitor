...
# IP address for the pico
pico_ip = 'http://192.168.0.234/'
...
while True:
    ...
    for item in info:
        ...
        elif item.Identifier == "/nvidiagpu/0/smalldata/3":
            GPU_RAM_total = item.Value
            data["GPU_RAM_total"] = GPU_RAM_total
        elif item.Identifier == "/nvidiagpu/0/smalldata/2":
            GPU_RAM_used = item.Value
            data["GPU_RAM_used"] = GPU_RAM_used
    ...
    for key, value in data.items():
        ...
        httpdata += 'S'
        httpdata += key
        httpdata += f'{round(value,2)}'
        httpdata += 'E'
        httpdata += key

    # print(httpdata)

    request = f"{pico_ip}/{httpdata}"

    try:
        r = requests.get(request, timeout=2.00)
        print(f'r:')
        print(str(r))
    except:
